...
2. **解析技术栈**：先读目标项目的 `README.md`。若它已记录技术栈（`## Tech stack` 段或
   `**Tech stack**:` 行），直接采用，不再询问；否则跑一次简短问卷（项目名、对话语言、前端框架、
   后端框架、数据库、对外端口）。不问设计细节——那是之后 auto-mattpocock grill 流程的事。
